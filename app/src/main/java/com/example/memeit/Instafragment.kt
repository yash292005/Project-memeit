...
class Instafragment : Fragment(), WallPaperAdapter.WallPaperClicked {
    ...
    override fun onCreateView(
        ...
        val Fab=  someview.findViewById<FloatingActionButton>(R.id.Fab)!!
    ...
    private fun fetchWallpaper(SearchQuery: String) {
        ...
        val jsonObjectRequest = object : JsonObjectRequest(
            ...
            { response ->
                ...
                if (WallPaperJsonArray.length() <= 6){
    ...
    private fun fetchWallPaperOnSearch(SearchQuery: String){
        val url = "https://api.pexels.com/v1/search?query=$SearchQuery&per_page=80"
        wallpaperLoader?.visibility = View.VISIBLE
        val jsonObjectRequest = object : JsonObjectRequest(
            Method.GET, url, null,
            { response ->
                ...
                mWallpaperAdapter.updateWallpaperOnSearch(WallpaperArray)
                wallpaperLoader?.visibility = View.GONE
                if (WallPaperJsonArray.length() <= 6){
                    Toast.makeText(context, "no value found", Toast.LENGTH_SHORT).show()
                }
